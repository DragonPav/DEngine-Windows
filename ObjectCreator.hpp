...
public:
	...
	static Object3D createSphere(Vector3 pos, float radius, int sectorCount, int stackCount, Camera* camera, Texture* tex) {
		...
		if (camera->rendUtils->lighting != nullptr) {
			...
			ret.normalBufSize = sizeof(GLfloat) * normals.size();
		}
		ret.vertBufSize = sizeof(GLfloat) * vertices.size();
		ret.indexBufSize = sizeof(GLuint) * indices.size();
		ret.texBufSize = sizeof(GLfloat) * texCoords.size();
		normals.clear();
		vertices.clear();
		indices.clear();
		texCoords.clear();
